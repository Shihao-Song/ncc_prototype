...
void Clusters::minClusters(std::vector<Neuron>& snn)
{
              ...
              cur_neuron_idx++)
    {
        ...
        if (snn[cur_neuron_idx].numInputNeurons()) 
        {
            ...
            // Iterate the clusters and find the cluster that can pack the most
            while (true)
            {
                unsigned size_before_work = non_unrolled_inputs.size();

                UINT64 chosen_cluster = INVALID_ID;
                unsigned num_to_pack = 0;
                for (auto &cluster : clusters)
                {
                    UINT64 cid = cluster->getClusterId();

                    unsigned tmp = numCanBePacked(cid,
                                                  non_unrolled_inputs);
                    if (tmp > num_to_pack) { chosen_cluster = cid; num_to_pack = tmp; }
                }

                if (chosen_cluster != INVALID_ID)
                {
                    packToCluster(num_to_pack,
                                  snn[cur_neuron_idx].getNeuronId(),
                                  chosen_cluster,
                ...
                unsigned size_after_work = non_unrolled_inputs.size();
                if (size_after_work == size_before_work) { break; }
            }
